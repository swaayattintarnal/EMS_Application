...
import { ArrowLeft, Plus, Search, Users, Calendar, Phone, Mail, Briefcase, GraduationCap, CreditCard, FileText, MapPin, HandIcon } from 'lucide-react'; 
...
const DeepEigen = () => {
  ...
  // Helper to get category and status counts 
  const getCategoryCount = (category, status = 'Current') => {
    return employees.filter(e => String(e.company) === 'DeepEigen' && String(e.category) === category && String(e.status) === status).length;
  };

  // Categories for filtering employees
  const categories = [
    { id: 'all', name: 'All Employees', count: employees.filter(e => String(e.company) === 'DeepEigen').length },
    ...
    { id: 'notice period', name: 'Notice Period Employees', count: employees.filter(e => String(e.company) === 'DeepEigen' && String(e.status) === 'Notice Period').length }, 
    { id: 'contractual', name: 'Contractual (Working)', count: getCategoryCount('Contractual') },
    { id: 'left', name: 'Left Interns', count: employees.filter(e => String(e.company) === 'DeepEigen' && String(e.status) === 'Left').length }, 
   
    { id: 'resigned', name: 'Resigned Employees', count: employees.filter(e => String(e.company) === 'DeepEigen' && String(e.status) === 'Resigned').length }, 
  ];


  const filteredEmployees = employees.filter(employee => {
    const matchesCompany = String(employee.company) === 'DeepEigen';
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div className="flex items-center">
                ...
                <div>
                  <h1 className="text-3xl font-bold">DeepEigen</h1>
                  <p className="text-green-200">Employee Management System</p>
                </div>
              </div>
            </div>
            ...
            <button
              ...
              className="flex items-center px-6 py-3 bg-white text-green-600 rounded-full font-semibold hover:bg-green-50 transition-colors duration-200">
          ...
          {filteredEmployees.map(employee => (
            <div
              ...
              <div className="text-center">
                ...
                <p className="text-green-600 font-medium mb-2">{employee.designation || 'N/A'}</p>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${String(employee.category) === 'Full-time' ? 'bg-green-100 text-green-800' : 
                      String(employee.category) === 'Intern' ? 'bg-blue-100 text-blue-800' : 
                        String(employee.category) === 'Contractual' ? 'bg-orange-100 text-orange-800' : 
